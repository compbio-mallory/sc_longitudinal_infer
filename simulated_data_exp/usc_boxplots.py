...
def plot_multipleBp(algo_metric_dict, data_list, y_max, y_min, output):
    ...
    boxplot_ForMultipleMetric('Unobserved Subclones',algo_metric_dict.get('lgTree_cusc'),algo_metric_dict.get('lgTree_iusc'),data_list,y_max,y_min,output,axes)
    ...
    fig.savefig('boxplots/usc/'+output+'.png',bbox_inches="tight")
...
def boxplot_ForMultipleMetric(metric, lgTree_data, lace_data, data_list, y_max, y_min, output, axis):
    ...
    axis.plot([], c='#e41a1c', label='Correct unboserved subclones')
    axis.plot([], c='#377eb8', label='Incorrect unboserved subclones')
    ...
    #elif metric == "Unobserved Subclones":
            ...
            if "eval" in file:
                ...
                with open(eval_path,'r') as f:
                    ...
                    for line in lines:
                        ...
                        if "Correct usc" in line:
                            line_list = line.split(" ")
                            print(" Line list ",line_list[8])
                            correct_usc = float(((line.split(" "))[8]).strip())
                            incorrect_usc = float(((line.split(" "))[13]).strip())
                            total_correct_usc = total_correct_usc+correct_usc
                            total_incorrect_usc = total_incorrect_usc+incorrect_usc
                ...
                cusc_key_name = sub_keyname+"_cusc"
                iusc_key_name = sub_keyname+"_iusc"
                ...
                if algo == "LACE" and sub_keyname == "t5":
                    ...
                    if cusc_key_name in dictName:
                        temp_list = dictName[cusc_key_name]
                        temp_list.append(total_correct_usc)
                        dictName[cusc_key_name] = temp_list
                    else:
                        cusc_list = []
                        vmeas_list.append(total_correct_usc)
                        dictName[cusc_key_name] = cusc_list

                    if iusc_key_name in dictName:
                        temp_list = dictName[iusc_key_name]
                        temp_list.append(total_incorrect_usc)
                        dictName[iusc_key_name] = temp_list
                    else:
                        iusc_list = []
                        vmeas_list.append(total_incorrect_usc)
                        dictName[iusc_key_name] = iusc_list
...
def create_lists(algoDict,data_list):
    ...
    for data in data_list:
        ...
        cusc_v = [val for key, val in cusc_dict.items() if data == (key.split('_'))[0]]
        cusc_list.append(cusc_v[0])
        iusc_v = [val for key, val in iusc_dict.items() if data == (key.split('_'))[0]]
        iusc_list.append(iusc_v[0])
    ...
    return prec_list, recall_list, vmeas_list, time_list, cusc_list, iusc_list, max_time, min_time
...
for algo in algo_list:
    ...
    prec_list, recall_list, vmeas_list, time_list, cusc_list, iusc_list, max_time, min_time = create_lists(algo_dict,data_list)
    ...
    algo_metric_dict[algo+"_cusc"] = cusc_list
    algo_metric_dict[algo+"_iusc"] = iusc_list
