...
def create_rc_job_file(fname,n,mpc,p,cluster_algo,sim):
    ...
    for lt in lines_together:
        with open(op_fname+"."+str(count)+".slurm",'w') as f:
            ...
            f.write("#SBATCH --output=./job_output/output"+str(count)+".out\n")
            f.write("#SBATCH --error=./job_output/error"+str(count)+".out\n")
            for st in lt:
                if st == "#":
                    continue
                f.write(st)
...
#            if cluster_algo == 'scite':
#                opdir_name = "./job_output"
...
#            elif 'output' in item and cluster_algo != 'scite':
                #print(" Output ",item)
...
#if args.algo == 'robustclone':
create_rc_job_file(args.input, args.n, args.mem_per_cpu, args.p, args.algo, args.sim)
#else:
