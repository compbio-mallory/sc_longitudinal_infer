...
def drawTree(tp_nodes, id_cells, id_newMut, id_plMut, id_backMut, id_edges, usc_nodes, name, sample):
    ...
    for tp, nodes in tp_nodes.items():
        if tp == -1:
            gNodeIds[-1] = "Root"
            graph.add_node(pydot.Node("Root", shape="circle", fixedsize="true", width=0.7))
            continue
        ...
        for n in nodes:
           ...
           #     firstNode = False
            if n in usc_nodes: # unobserved subclones are not added in the subgraph
                graph.add_node(pydot.Node(nodeId, shape="circle", style="dotted", fixedsize="true", width=0.7))
            else:
                n2 = pydot.Node(nodeId, shape="circle", color = colorVal, style="filled", fixedsize="true", width=0.7)
                S.add_node(n2)
    ...
    for node, edge in id_edges.items():
        ...
        if id_newMut[node] != set():
            ...
            if node in id_backMut:
                ...
                #graph.add_edge(pydot.Edge(p_gNode, gNode, color="black", label="<<font color='black'>"+mutations+"</font><font color='red'>"+backMut+"</font>>"))
                graph.add_edge(pydot.Edge(p_gNode, gNode, color="black", label=mutations))
                graph.add_edge(pydot.Edge(p_gNode, gNode, color="red", label=backMut, fontcolor="red"))
                if smutList != []:
                    graph.add_edge(pydot.Edge(p_gNode, gNode, color="blue", label=smut, fontcolor="blue"))
            else:
                #if node in id_plMut:
                #    id_plMut[node].sort()
            ...
            if node in id_backMut:
                backMut = ";".join(str(i) for i in id_backMut[node])
                graph.add_edge(pydot.Edge(p_gNode, gNode, color="black", label=backMut, fontcolor="red"))
            else:
                graph.add_edge(pydot.Edge(p_gNode, gNode, color="black"))
