...
def parse_args():
    ...
    # our additon to take user argument whether to run singlet or doublet mode
    parser.add_argument(
        '--running_doublet_mode', action='store_true', default=False,
        help='Run doublet detetion if set to True else singlet code.'
    )
    ...
    # our additon
    model.add_argument(
        '-MR', '--missingRate', type=float, default=-1,
        help='Fixed error rate for missing values.'
    )
    model.add_argument(
    ...
    # mcmc.add_argument(
    #     '-s', '--steps', type=int, default=500,
    #     help='Number of MCMC steps. Default = 500.'
    # )
    mcmc.add_argument(
...
def main(args):
    ...
    #  args.DPa_prior = [-1, -1]
    #  args.param_prior = [0.1, 0.1]
    # ------------------------------------------------------------------------------
    # our additon to run doublet mode
    # ------------------------------------------------------------------------------
    if args.running_doublet_mode:
        if args.falsePositive > 0 and args.falseNegative > 0:
            args.error_update_prob = 0
            import libs.doublet as doublet
            BnpC = doublet.CRP_doublet(
                data, DP_alpha=args.DPa_prior, param_beta=args.param_prior,
                FN_error=args.falseNegative, FP_error=args.falsePositive,
                Missing_error=args.missingRate,
            )
        
        else:
            # need to work here too to update error rates in doublet mode
    ...
    # BnpC.get_ll_full_error(args.falsePositive, args.falseNegative) 
