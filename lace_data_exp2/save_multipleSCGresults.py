...
def save_multipleSCGresults(config_params, fileName, clusterNo_list, nIters):
    for i in clusterNo_list:
        cluster_update = {'num_clusters': int(i)}
        ...
        with open('../../../SCG_Roth/scg/examples/new_config.yaml', 'w') as fp:
            documents = yaml.dump(config_params, fp)

        for j in range(1, int(nIters)):
            print("Iteration ",j)
...
parser.add_argument("-cluster_list", "--cluster_list",nargs='+', dest = "cluster_list", help="List of cluster numbers")
