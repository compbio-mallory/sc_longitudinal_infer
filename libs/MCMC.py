...
class Chain():
    ...
    def stdout_progress(self):
        ...
        self.MH_counter = np.zeros((5, 2))
        print("self.MH_counter", self.MH_counter)
    ...
    def do_step(self):
        ...
        if not self.fix_assign:
            if np.random.random() < self.mcmc['sm_prob']:
                print("update_assignments_split_merge")
        ...
        # print("Final parameter", self.model.parameters)
        ...
        if self.learning_errors and np.random.random() < self.mcmc['error_prob']:
            print("Calling to update errors")
            # import pdb
            # pdb.set_trace()
