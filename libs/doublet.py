...
class CRP_doublet:
    ...
    Arguments:
        ...
        Missing_error (float): Fixed missing rate
    """
    def __init__(self, data, DP_alpha=-1, param_beta=[1, 1], FN_error=EPSILON,
                FP_error=EPSILON, Missing_error=EPSILON):
        # Fixed data
        self.data = data
        ...
        # print("_beta_mix_const: {}\n".format(self._beta_mix_const))

        # Error rates
        self.FP = FP_error
        self.FN = FN_error
        self.MR = Missing_error
        ...
        self.DP_a_prior = gamma_fct(*self.DP_a_gamma) # this is gamma function
        self.DP_a = np.sqrt(self.cells_total) # need to ask to writer why the initial value of alpha_not is np.sqrt(self.cells_total)
        # print("self.DP_a: {}\n".format(self.DP_a))

        # Flexible data - Initialization
        self.CRP_prior = None
        self.assignment = None # primary cluster indicator
        self.secondary_assignment = None # secondary cluster indicator
        ...
        # ------------------------------------------------------------------------------
        # our addition for doublet code
        # ------------------------------------------------------------------------------
        self.delta = beta.rvs(self.p, self.q, size=1) # doublet rate with beta prior
        self.bernoulli_y = bernoulli.rvs(p=self.delta, size=self.cells_total) # bernoulli y
    ...
    # This is the equation for beta interms of gamma
    @staticmethod
    def beta_fct(p, q):
    ...
    def log_CRP_prior(n_i, n, a, dtype=np.float64):
        return np.log(n_i, dtype=dtype) - np.log(n - 1 + a, dtype=dtype) # loga -logb = log(a/b)


    @staticmethod
    def _normalize_log_probs(probs):
        max_i = bn.nanargmax(probs) # Indices of maximum values along axis, ignoring NaNs.
        try:
            exp_probs = np.exp(probs[np.arange(probs.size) != max_i]  - probs[max_i])   
                # np.arange(3) = array([0, 1, 2])
                # np.exp = calculates e^x for each x in your list 
        except FloatingPointError:
            exp_probs = np.exp(
                np.clip(probs[np.arange(probs.size) != max_i] - probs[max_i],
                    log_EPSILON, 0)
            )
        probs_norm = probs - probs[max_i] - np.log1p(bn.nansum(exp_probs)) # np.log1p + natural logarith value
    ...
    def init(self, mode='random', assign=False):
        ...
        if assign:
            ...
            cl, cl_size = np.unique(assign, return_counts=True) # Find the unique elements of an array, return_counts=True  return the number of times each unique item appears in ar
            for i in range(cl.size):
                bn.replace(self.assignment, cl[i], i) # all the elements in self.assignment with value cl[i] is replaced with i
        ...
        # Complete random
            ...
            # cells randomly assigned to cluster
            ...
            for i in range(cl.size):
                ...
                self.cells_per_cluster[i] = cl_size[i]
            # print("after self.assignment", self.assignment)
            self.parameters = self._init_cl_params(mode)
            # our additon secondar clustor indicator
            self.secondary_assignment = np.random.uniform(0, high=cl.size, size=self.cells_total)
        else:
    ...
    def doublet_detection(self):
        self.doublet_result = {}
        for cell in self.cells_total:
            if self.bernoulli_y[cell] ==1:
    ...
    def _init_cl_params(self, mode='random', fkt=1):
        ...
        if mode == 'separate':
            ...
            # print("params[k]", params[0])
            # print("params[k]", params[99])
    ...
    def init_DP_prior(self):
        # print("calling init_DP_prior!!!!!")
        cl_vals = np.append(np.arange(1, self.cells_total + 1), self.DP_a)
        CRP_prior = self.log_CRP_prior(cl_vals, self.cells_total, self.DP_a) # self.log_CRP_prior is first CRP equation
        self.CRP_prior = np.append(0, CRP_prior)
        # print("self.CRP_prior ", self.CRP_prior )
        # print("cl_vals", cl_vals)
        ...
        for i in range(self.muts_total):
            for j in range(self.cells_total):
                if math.isnan(self.data[i][j]):
                    sum = sum + math.log(self.MR)      
        if flat:
            return bn.nansum(ll_full) + sum
        else:
            # return bn.nansum(ll_full, axis=1) 
            return bn.nansum(ll_full, axis=1) + sum


    def _Bernoulli_FN(self, x):
        return (1 - self.FN - self.MR) ** x * self.FN ** (1 - x)


    def _Bernoulli_FP(self, x):
        return (1 - self.FP - self.MR) ** (1 - x) * self.FP ** x
    ...
    def get_lpost_single_new_cluster(self):
        ...
        ll_full = np.log(ll_FN + ll_FP)
        for i in range(self.cells_total):
            for j in range(self.muts_total):
                if math.isnan(self.data[i][j]):
                    sum = sum + math.log(self.MR)
        return bn.nansum(ll_full, axis=1) + sum + self.CRP_prior[-1]
...
# ------------------------------------------------------------------------------
    ...
    def do_split_move(self, step_no=5):
        ...
        # print("cluster_size: {}\n".format(cluster_size))
        # print("cluster_size.sum(): {}\n".format(cluster_size.sum()))
